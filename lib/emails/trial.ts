...
export function nurtureD3Email(c: TrialEmailCtx): { subject: string; html: string } {
  ...
  if (c.lang === 'vi') {
    return {
      ...
      html: shell(`
        ...
        <p style="${P}">Một tập đoàn truyền thông hàng đầu tại Việt Nam đo hiện diện AI hàng tuần trên 5 công cụ, theo dõi những chỗ câu trả lời AI lệch khỏi thông điệp chuẩn của mình, và tăng chỉ số +6,8% trong tháng đầu vận hành đầy đủ.</p>
  ...
  return {
    ...
    html: shell(`
      ...
      <p style="${P}">A leading Vietnamese media group measures its AI presence weekly across 5 engines, tracks where AI answers diverge from its approved messaging, and lifted its index +6.8% in the first full month on the platform.</p>
